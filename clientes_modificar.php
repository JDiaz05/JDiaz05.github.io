...
$sql=$conexion->query("SELECT * FROM clientes WHERE id_c=$id");
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<div class="container mt-5">
                    ...
                    <h4 class="text-center alert alert-secondary">Modificar cliente</h4>
                     <input type="hidden" name="id" value="<?=$_GET['id']?>">

						<?php
                        include 'clientes_actualizar.php';
                      while($datos=$sql->fetch_object()) { ?>
                        ...
                        <b><p>Nota/comentario  (max 50 caracteres):</p></b>
                        <input type="text" class="form-control mb-3" name="comentarios" placeholder="Nota/comentario" value="<?=$datos->comentarios?>" maxlength="50">
                     <?php } 
                      ?>


                    <button type="submit" class="btn btn-primary btn-block" name= "btnmodificar" value="ok">Modificar cliente</button>
                    <button type="submit" class="btn btn-danger btn-block" name= "btnclientes" value="ok">Volver</button>
</div>
</body>
</html>